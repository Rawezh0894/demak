<?php
// Update Interior Design Project Functionality
...
try {
    ...
    if (isset($_FILES['main_image']) && $_FILES['main_image']['error'] === UPLOAD_ERR_OK) {
        $upload_dir = '../../assets/images/projects/interior_design/';
    ...
    if (isset($_FILES['additional_images']) && !empty($_FILES['additional_images']['name'][0])) {
        $upload_dir = '../../assets/images/projects/interior_design/gallery/';
        if (!file_exists($upload_dir)) {
            mkdir($upload_dir, 0755, true);
        }
    ...
    $stmt = $pdo->prepare("
        UPDATE interior_design_projects 
        SET {$update_fields}
    ...
    if (!empty($additional_images)) {
        $image_stmt = $pdo->prepare("
            INSERT INTO interior_design_images (project_id, image_path, is_main, sort_order) 
            VALUES (?, ?, 0, ?)
        ");
        
        // Get current max sort_order
        $max_stmt = $pdo->prepare("SELECT COALESCE(MAX(sort_order), 0) FROM interior_design_images WHERE project_id = ?");
    ...
    if (function_exists('createDetailedNotification')) {
        createDetailedNotification($pdo, $_SESSION['admin_id'], 'update', 'interior_design_projects', $project_id, 'Interior design project updated', null, null, ['project_name' => $project_name], getUserIP());
    }
