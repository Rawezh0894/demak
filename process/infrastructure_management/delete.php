...
try {
    ...
    $stmt = $pdo->prepare("SELECT name FROM infrastructure_projects WHERE id = ? AND created_by = ?");
    ...
    // Delete main image
    $stmt = $pdo->prepare("SELECT main_image FROM infrastructure_projects WHERE id = ?");
    $stmt->execute([$project_id]);
    ...
    $stmt = $pdo->prepare("DELETE FROM infrastructure_projects WHERE id = ? AND created_by = ?");
    $stmt->execute([$project_id, $_SESSION['admin_id']]);
    ...
    if (function_exists('createDetailedNotification')) {
        createDetailedNotification($pdo, $_SESSION['admin_id'], 'delete', 'infrastructure_projects', $project_id, 'Infrastructure project deleted', null, null, ['project_name' => $project_name], getUserIP());
    }
