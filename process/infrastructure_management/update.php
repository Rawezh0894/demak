...
try {
    ...
    // Update project
    $update_fields = "category_key = ?, name = ?, name_ku = ?, name_ar = ?, 
    ...
    // Update features
    $pdo->prepare("DELETE FROM project_features WHERE project_id = ?")->execute([$project_id]);
    ...
    if (function_exists('createDetailedNotification')) {
        createDetailedNotification($pdo, $_SESSION['admin_id'], 'update', 'infrastructure_projects', $project_id, 'Infrastructure project updated', null, null, ['project_name' => $project_name], getUserIP());
    }
