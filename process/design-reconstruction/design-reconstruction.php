...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== $_SESSION['csrf_token']) {
        ...
        // Use action + project_id (if exists) to create unique key
        $unique_id = isset($_POST['project_id']) && !empty($_POST['project_id']) ? $_POST['project_id'] : 'new';
        ...
        if (!$recent_action_found) {
            ...
            try {
                ...
                switch ($_POST['action']) {
                    case 'add_project':
                        require_once '../../process/design-reconstruction/add.php';
                        break;
                    case 'edit_project':
                        require_once '../../process/design-reconstruction/update.php';
                        break;
                    case 'delete_project':
                        // Set flag to capture response instead of direct echo
                        $GLOBALS['delete_response_capture'] = true;
                        $delete_response = require '../../process/design-reconstruction/delete.php';
                        unset($GLOBALS['delete_response_capture']);
                        
                        // Validate response
                        if ($delete_response === null || !is_array($delete_response)) {
                            error_log("Delete response is null or invalid: " . var_export($delete_response, true));
                            echo json_encode(['success' => false, 'message' => 'Invalid response from delete operation']);
                            exit;
                        }
                        
                        // If delete was successful, remove action_key immediately
                        // because the project no longer exists and can't be deleted again
                        if (isset($delete_response['success']) && $delete_response['success'] === true) {
                            if (isset($_SESSION['processed_actions'][$action_key])) {
                ...
                // Keep the action key for 3 seconds to prevent immediate duplicate submissions (except delete)
...
// Load data
require_once '../../process/design-reconstruction/select.php';
?>