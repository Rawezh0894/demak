<?php
// Select and Load Data Functionality for Exterior Design
...
// Load exterior design projects from database with pagination
$exterior_design_projects = [];
$total_projects = 0;
$total_pages = 0;
...
try {
    ...
    $exterior_design_projects = $stmt->fetchAll(PDO::FETCH_ASSOC);
    
    // Load additional images for each project
    foreach ($exterior_design_projects as &$project) {
...
function getExteriorProjectById($pdo, $project_id, $admin_id) {
    try {
        $stmt = $pdo->prepare("
            SELECT * FROM exterior_design_projects
...
function searchExteriorProjects($pdo, $search_term) {
    try {
        $stmt = $pdo->prepare("
            SELECT * FROM exterior_design_projects
...
function getExteriorProjectStatistics($pdo) {
    try {
        $stats = [];
        
        // Total projects
        $stmt = $pdo->query("SELECT COUNT(*) as total FROM exterior_design_projects WHERE is_active = 1");
